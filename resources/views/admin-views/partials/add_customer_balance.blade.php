    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{\App\CPU\translate('add_balance')}}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{route('admin.customer.update-balance')}}" method="post" class="add_balance_form row">
                    @csrf
                    <input type="hidden" id="customer_id" name="customer_id"  value="{{$customer['id']}}" />
                        ...
                        <div class="form-group col-12 col-sm-6">
                            ...
                            <input type="number" class="form-control current_balance" readonly
                            value="{{$customer['balance']}}" >
                        </div>
                        <div class="form-group col-12 col-md-6">
                            <label for="">{{\App\CPU\translate('amount')}}</label>
                            <input type="number" step="0.01" min="0" class="form-control add_balance" name="amount" required>
                        </div>
                        <div class="form-group col-12 col-md-6">
                            <label for="">{{\App\CPU\translate('remaining_balance')}}</label>
                            <input type="number" readonly class="form-control remaining_balance" />
                        </div>
